...
Analysis of Admissions: Inspecting the histogram of admissions shows that the data is right-skewed, also known as positively skewed. the Admissions variable was evaluated to determine whether this skew is caused by outliers or not. Outlying values for admissions statistics needs to be based on a variety of factors, since they can be affected by factors such as county and the type of drug use being treated. 

Analysis of Year: The distribution of admissions per year was examined. Based on the summary table, overall number of clinics reporting per year stayed consistent which no major variance between years.

Analysis of Service type: Across the entire dataset, the service "non-med sup chem depend op" was only used once.

Analysis of primary substance: There is a category called 'none' with almost no data in it that can probably be removed.
...
ggplot(data = addiction_clean, mapping = aes(x = Admissions)) +
  ...
  theme_bw()
#-------------------------------------------------------------------------------
#Analyze Dataset - Years

table(addiction_clean$Year)

addiction_year <- addiction_clean %>%
  group_by(Year) %>%
  summarise(sum(Admissions))

ggplot(data = addiction_year, mapping = aes(x = Year, y = `sum(Admissions)`)) +
  geom_col(binwidth = 1, fill = "darkcyan") + 
  labs(
    title = "Admisssions per Year",
    caption = "Source: OASAS Chemical Dependance Treatment Program"
  ) +
  xlab("Year") +
  ...
  theme_bw()
#-------------------------------------------------------------------------------
#Analyze Dataset - County.of.Program.Location

addiction_County <- addiction_clean %>%
  group_by(County.of.Program.Location) %>%
  summarise(sum(Admissions))

ggplot(data = addiction_County, mapping = aes(x = reorder(County.of.Program.Location, desc(`sum(Admissions)`)), y = `sum(Admissions)`)) +
  geom_col(stat = "identity", fill = "darkcyan") + 
  labs(
    title = "Admisssions per County",
    caption = "Source: OASAS Chemical Dependance Treatment Program"
  ) +
  xlab("County") +
  ylab("Admissions per County") +
  scale_y_log10() +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
#-------------------------------------------------------------------------------
#Analyze Dataset - Program Category

addiction_program <- addiction_clean %>%
  group_by(Program.Category) %>%
  summarise(sum(Admissions))

ggplot(data = addiction_program, mapping = aes(x = reorder(Program.Category, desc(`sum(Admissions)`)), y = `sum(Admissions)`)) +
  geom_col(stat = "identity", fill = "darkcyan") + 
  labs(
    title = "Admisssions per Program",
    caption = "Source: OASAS Chemical Dependance Treatment Program"
  ) +
  xlab("Program") +
  ylab("Admissions per Program") +
  scale_y_log10() +
  theme_bw()
#-------------------------------------------------------------------------------
#Analyze dataset - Service type

addiction_service <- addiction_clean %>%
  group_by(Service.Type) %>%
  summarise(sum(Admissions))

ggplot(data = addiction_service, mapping = aes(x = reorder(Service.Type, desc(`sum(Admissions)`)), y = `sum(Admissions)`)) +
  geom_col(stat = "identity", fill = "darkcyan") + 
  labs(
    title = "Admisssions per Service Type",
    caption = "Source: OASAS Chemical Dependance Treatment Program"
  ) +
  xlab("Service Type") +
  ylab("Admissions per Service Type") +
  scale_y_log10() +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
#-------------------------------------------------------------------------------
#Analyze data - Age Group

addiction_age <- addiction_clean %>%
  group_by(Age.Group) %>%
  summarise(sum(Admissions))

ggplot(data = addiction_age, mapping = aes(x = reorder(Age.Group, desc(`sum(Admissions)`)), y = `sum(Admissions)`)) +
  geom_col(stat = "identity", fill = "darkcyan") + 
  labs(
    title = "Admisssions by Age Group",
    caption = "Source: OASAS Chemical Dependance Treatment Program"
  ) +
  xlab("Age Group") +
  ylab("Admissions by Age Group") +
  theme_bw()
#-------------------------------------------------------------------------------
#Analyze data - Primary Substance Group

addiction_substance <- addiction_clean %>%
  group_by(Primary.Substance.Group) %>%
  summarise(sum(Admissions))

ggplot(data = addiction_substance, mapping = aes(x = reorder(Primary.Substance.Group, desc(`sum(Admissions)`)), y = `sum(Admissions)`)) +
  geom_col(stat = "identity", fill = "darkcyan") + 
  labs(
    title = "Admisssions by Substance",
    caption = "Source: OASAS Chemical Dependance Treatment Program"
  ) +
  xlab("Primary Substance") +
  ylab("Admissions by Substance") +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
#------------------------------
...
Within the county data frame county, the boroughs of NYC are specified by their county code 'NY' and the borough name in parentheses in order to create a distinctive key:value relationship. 
