...
addiction_df <- read.csv("https://data.ny.gov/api/views/ngbt-9rwf/rows.csv?accessType=DOWNLOAD&sorting=true")
...
addiction_clean$Year <- as.Date(addiction_clean$Year, origin = "2007")
#-------------------------------------------------------------------------------

#Analyze Dataset - Heroin Admittance state-wide.

addiction_heroin_statewide <- addiction_clean %>%
  filter(Primary.Substance.Group == "Heroin") %>%
  group_by(Year) %>%
  summarise(sum(Admissions))

#view(addiction_heroin)

ggplot(data = addiction_heroin_statewide, mapping = aes(x = Year, y = `sum(Admissions)`, fill = Year)) +
  geom_col() + 
  labs(
    title = "Heroin Admittance State-Wide",
    Caption = "Source: OASAS Chemical Dependance Treatment Program", 
  ) + 
  xlab("Year") +
  ylab("State-wide Admissions") +
  theme_bw()

#-------------------------------------------------------------------------------

#Analyze Dataset - Crisis Admissions by County over timespan. 

addiction_heroin_Crisis <- addiction_clean %>%
  filter(Program.Category == "Crisis") %>%
  group_by(Year) %>%
  summarise(sum(Admissions))
