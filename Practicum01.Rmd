...
summary(addiction_outlier)

addiction_outlier_county <- addiction_outlier %>%
  group_by(County.of.Program.Location) %>%
  summarise(sum(Admissions)) %>%
  rename("Admissions" = `sum(Admissions)`)

ggplot(data = addiction_outlier_county, mapping = aes(x = reorder(County.of.Program.Location, desc(Admissions)), y = Admissions)) + 
  geom_bar(stat = "identity") +
  labs(
    title = "Figure 1: Outlying admissions values by County"
  ) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
#-------------------------------------------------------------------------------

addiction_outlier_substance <- addiction_outlier %>%
  group_by(Primary.Substance.Group) %>%
  summarise(sum(Admissions)) %>%
  rename("Admissions" = `sum(Admissions)`)

ggplot(data = addiction_outlier_substance, mapping = aes(x = reorder(Primary.Substance.Group, desc(Admissions)), y = Admissions)) + 
  geom_bar(stat = "identity") +
  labs(
    title = "Figure 1: Outlying admissions values by Substance"
  ) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
```
