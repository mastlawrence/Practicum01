...
Create a new data frame which groups various quality events by analyst to prepare for modeling.

```{r}
hans_QEM <- hans %>%
  group_by(Analyst) %>%
    summarise(sum(missed_QEM), sum(incorrect_result_reported), sum(reprocessing_Integration),
              sum(reprocessing_calculation), sum(LIMS_incompletation), sum(GDP_misses), 
              sum(missing_client_documentation))
#---------------------------------------------------------------------------------------------
vir_QEM <- vir %>%
  group_by(Analyst) %>%
    summarise(sum(missed_QEM), sum(incorrect_result_reported), sum(reprocessing_Integration),
...
```
  ...
  Prepare bar charts which show number of GDP misses per analyst per tech reviewers.
  
```{r}
#Hans
ggplot(data = hans_QEM, mapping = aes())
```
